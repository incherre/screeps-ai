...
function wrapFunction(obj: object, key: PropertyKey, className?: string) {
  ...
  const memKey = className + `:${key as any}`;
...
function record(key: string | symbol, time: number) {
  if (!Memory.profiler.data[key as any]) {
    Memory.profiler.data[key as any] = {
      calls: 0,
      time: 0,
    };
  }
  Memory.profiler.data[key as any].calls++;
  Memory.profiler.data[key as any].time += time;
}
...
function outputProfilerData() {
  ...
  const data = Reflect.ownKeys(Memory.profiler.data).map((key) => {
    calls = Memory.profiler.data[key as any].calls;
    time = Memory.profiler.data[key as any].time;
    result = {};
    result.name = `${key as any}`;
    result.calls = calls;
